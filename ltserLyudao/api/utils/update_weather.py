...
metadata_url = (
    "https://data.depositar.io/api/3/action/package_show?id=ltser-lyudao-weather"
)
...
try:
    table_name = WeatherData._meta.db_table
            ...
            if response.status_code == 200:
                ...
                for record in records:
                    ...
                    time = validate_datetime(
                        record.get("eventDate"), "eventDate", record
                    )
                    ...
                    PAR = validate_decimal(record.get("PAR"), "PAR", record)
                    SolarRadiation = validate_decimal(
                        record.get("SolarRadiation"), "SolarRadiation", record
                    )
                    WindDirection = validate_decimal(
                        record.get("WindDirection"), "WindDirection", record
                    )
                    Pressure = validate_decimal(
                        record.get("Pressure"), "Pressure", record
                    )
                    WindSpeed = validate_decimal(
                        record.get("WindSpeed"), "WindSpeed", record
                    )
                    GustSpeed = validate_decimal(
                        record.get("GustSpeed"), "GustSpeed", record
                    )
                    airTemperature = validate_decimal(
                        record.get("airTemperature"), "airTemperature", record
                    )
                    RH = validate_decimal(record.get("RH"), "RH", record)
                    precipitation = validate_decimal(
                        record.get("precipitation"), "precipitation", record
                    )
                    ...
                    data_list.append(
                        WeatherData(
                            ...
                            stationAttribute=record.get("stationAttribute"),
                            decimalLongitude=decimalLongitude,
                            decimalLatitude=decimalLatitude,
                            stationAddress=record.get("stationAddress"),
                            PAR=PAR,
