...
metadata_url = 'https://data.depositar.io/api/3/action/package_show?id=ltser-lyudao-weather'
        ...
        while not total_count:
            ...
            if response.status_code == 200:
                ...
                for record in records:
                    ...
                    SolarRadiation = validate_decimal(record.get('SolarRadiation'), 'SolarRadiation', record)
                    WindDirection = validate_decimal(record.get('WindDirection'), 'WindDirection', record)
                    Pressure = validate_decimal(record.get('Pressure'), 'Pressure', record)
                    WindSpeed = validate_decimal(record.get('WindSpeed'), 'WindSpeed', record)
                    GustSpeed = validate_decimal(record.get('GustSpeed'), 'GustSpeed', record)
                    airTemperature = validate_decimal(record.get('airTemperature'), 'airTemperature', record)
                    RH = validate_decimal(record.get('RH'), 'RH', record)
                    precipitation = validate_decimal(record.get('precipitation'), 'precipitation', record)
