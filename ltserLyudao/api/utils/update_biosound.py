...
print("Database: BioSoundData")

# Step 1: Fetch metadata from API
metadata_url = (
    "https://data.depositar.io/api/3/action/package_show?id=ltser-lyudao-biosound"
)
...
try:
    table_name = BioSoundData._meta.db_table
        ...
        while not total_count:
            ...
            if response.status_code == 200:
                ...
                for record in records:
                    ...
                    classid = validate_integer(record.get("classid"), "classid", record)
                    time_begin = validate_integer(
                        record.get("time_begin"), "time_begin", record
                    )
                    time_end = validate_integer(
                        record.get("time_end"), "time_end", record
                    )
                    freq_low = validate_integer(
                        record.get("freq_low"), "freq_low", record
                    )
                    freq_high = validate_integer(
                        record.get("freq_high"), "freq_high", record
                    )
                ...
                try:
                    BioSoundData.objects.bulk_create(data_list)
                except Exception as e:
                    print(
