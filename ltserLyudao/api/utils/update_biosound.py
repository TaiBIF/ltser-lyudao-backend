...
metadata_url = 'https://data.depositar.io/api/3/action/package_show?id=ltser-lyudao-biosound'
...
try:
    BioSoundData.objects.all().delete()
        ...
        while not total_count:
            ...
            if response.status_code == 200:
                ...
                for record in records:
                    ...
                    freq_low = validate_integer(record.get('freq_low'), 'freq_low', record)
                    freq_high = validate_integer(record.get('freq_high'), 'freq_high', record)
