...
metadata_url = (
    "https://data.depositar.io/api/3/action/package_show?id=ltser-lyudao-water"
)
...
try:
    table_name = WaterData._meta.db_table
        ...
        while not total_count:
            ...
            if response.status_code == 200:
                ...
                for record in records:
                    ...
                    verbatimDepth = validate_decimal(
                        record.get("verbatimDepth"), "verbatimDepth", record
                    )
                    waterTemperature = validate_decimal(
                        record.get("waterTemperature"), "waterTemperature", record
                    )
                    lipid = validate_decimal(record.get("lipid"), "lipid", record)
                    conductivity = validate_decimal(
                        record.get("conductivity"), "conductivity", record
                    )
                    salinity = validate_decimal(
                        record.get("salinity"), "salinity", record
                    )
                    turbidity = validate_decimal(
                        record.get("turbidity"), "turbidity", record
                    )
                    ...
                    TBC = validate_decimal(record.get("TBC"), "TBC", record)
                    vibrio = validate_decimal(record.get("vibrio"), "vibrio", record)
                    COD = validate_decimal(record.get("COD"), "COD", record)
                    MBAS = validate_decimal(record.get("MBAS"), "MBAS", record)
                    TOC = validate_decimal(record.get("TOC"), "TOC", record)
                ...
                try:
                    WaterData.objects.bulk_create(data_list)
                except Exception as e:
                    print(
