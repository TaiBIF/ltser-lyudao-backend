...
from api.importing.mappings.weather_mapping import build_weather_payload


DEFAULT_WEATHER_HASH_FIELDS = [
    "dataID",
    "resourceName",
    "time",
    "locationID",
...
class WeatherDataAdapter:
    ...
    def __init__(self, hash_fields=None):
        self.hash_fields = hash_fields or list(DEFAULT_WEATHER_HASH_FIELDS)
        self.validators = {
    ...
    def build_payload(self, record):
        return build_weather_payload(record, self.validators)
    ...
    def fetch_existing_hash_map(self, keys):
        ...
        return {
            ...
            for data_id, pk, data_hash in WeatherData.objects.filter(
                dataID__in=keys
    ...
    def write(self, to_create, to_update):
        ...
        with transaction.atomic():
            if to_create:
                WeatherData.objects.bulk_create(to_create, batch_size=1000)
