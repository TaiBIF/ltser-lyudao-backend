...
from api.importing.mappings.streamdata_mapping import build_streamdata_payload


DEFAULT_STREAM_HASH_FIELDS = [
    "dataID",
    "eventID",
    "time",
    "locationID",
...
class StreamDataAdapter:
    ...
    def __init__(self, hash_fields=None):
        self.hash_fields = hash_fields or list(DEFAULT_STREAM_HASH_FIELDS)
        self.validators = {
    ...
    def build_payload(self, record):
        return build_streamdata_payload(record, self.validators)
    ...
    def fetch_existing_hash_map(self, keys):
        if not keys:
            return {}
        return dict(
            StreamData.objects.filter(dataID__in=keys).values_list(
                "dataID", "data_hash"
            )
        )

    def make_instance(self, payload):
        return StreamData(**payload)
    ...
    def write(self, to_create, to_update):
        ...
        with transaction.atomic():
            if to_create:
                StreamData.objects.bulk_create(to_create, batch_size=1000)

            if to_update:
                StreamData.objects.bulk_update(
                    to_update,
