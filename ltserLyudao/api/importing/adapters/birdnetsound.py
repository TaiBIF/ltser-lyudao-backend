...
from api.importing.mappings.birdnetsound_mapping import build_birdnet_sound_payload


DEFAULT_BIRDNET_SOUND_HASH_FIELDS = [
    "dataID",
    "eventID",
    "vernacularName",
    "model",
...
class BirdNetSoundDataAdapter:
    key_field = "dataID"

    def __init__(self, hash_fields=None):
        self.hash_fields = hash_fields or list(DEFAULT_BIRDNET_SOUND_HASH_FIELDS)
        self.validators = {
    ...
    def build_payload(self, record):
        return build_birdnet_sound_payload(record, self.validators)
    ...
    def fetch_existing_hash_map(self, keys):
        ...
        return {
            ...
            for data_id, pk, data_hash in BirdNetSoundData.objects.filter(
                dataID__in=keys
            ).values_list("dataID", "id", "data_hash")
        }

    def make_instance(self, payload):
        return BirdNetSoundData(**payload)
    ...
    def write(self, to_create, to_update):
        ...
        with transaction.atomic():
            if to_create:
                BirdNetSoundData.objects.bulk_create(to_create, batch_size=1000)

            if to_update:
                BirdNetSoundData.objects.bulk_update(
                    to_update,
