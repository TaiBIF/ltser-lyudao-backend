...
class FormLinkSerializer(serializers.ModelSerializer):
    attachments = FormLinkAttachmentSerializer(
        many=True, read_only=True, source="formLinkAttachments"
    )
...
class NewsDetailSerializer(serializers.ModelSerializer):
    ...
    attachments = NewsAttachmentSerializer(
        many=True, read_only=True, source="newsAttachments"
    )
    ...
    def get_cover(self, obj):
        if (
            obj.cover_image
            and obj.cover_image.image
            and hasattr(obj.cover_image.image, "url")
        ):
...
class AboutDetailSerializer(serializers.ModelSerializer):
    attachments = AboutAttachmentSerializer(
        source="aboutAttachments", many=True, read_only=True
    )
